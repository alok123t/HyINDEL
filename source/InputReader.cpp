...
void readInput(BamTools::BamReader &br, const int mean, const int stdDev) {
	...
	parseOutput(info_dels_large, info_dels_small, info_ins, pred_dels_large, pred_dels_small, pred_ins);
}

void processInput(const std::string filePath, const int mean, const int stdDev) {
    BamTools::BamReader br;

    if (!openInput(filePath, br)) return;
    
    readInput(br, mean, stdDev);
