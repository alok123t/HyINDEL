...
for arg; do
    ...
    -o)
        ...
        ;;
    -h)
        echo "Usage: bash /path/to/preProcess.sh -i /path/to/inputFile -o /path/to/outputFolder"
        exit 1
        ;;
    --)
        shift
...
# Remove tmp files
rm $OUTPUT_FOLDER"tmp0_chr_sizes.bed" $OUTPUT_FOLDER"tmp0_cov.regions.sort.bed"
